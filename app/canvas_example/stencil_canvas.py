...
class StencilCanvasApp(App):
    ...
    def reset_stenctil(self, wid, *largs):
        wid.pos = (0,0)
    ...
    def build(self):
        ...
        label = Label(text='0')
        btn_add200 = Button(text='+200 rects')
        btn_add200.bind(on_press = partial(self.add_rects, label, wid, 200))

        btn_reset = Button(text='Reset Rectangles')
        btn_reset.bind(on_press = partial(self.reset_rects, label, wid))

        btn_stenctil = Button(text='Reset Stencil')
        btn_stenctil.bind(on_press = partial(self.reset_stenctil, wid))

        layout = BoxLayout(size_hint=(1,None), height=50)
        layout.add_widget(btn_add200)
        layout.add_widget(btn_reset)
        layout.add_widget(btn_stenctil)
