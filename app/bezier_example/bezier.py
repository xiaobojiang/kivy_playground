...
class BezierTest(BoxLayout):
    def __init__(self, points=[], loop=False, *args, **kwargs):
        ...
        with self.canvas:
            ...
            Color(1.,0.,1.)
            self.line = Line(
                points=self.points+self.points[:2], #enclose the line by add first point as final point
                dash_offset=10,
                dash_length=100
            )
    ...
    def _set_bezier_dash_offset(self, instance, value):
        self.bezier.dash_length = int(100-value)
        self.bezier.dash_offset = int(value)
    
    def _set_line_dash_offset(self, instance, value):
        self.line.dash_length = int(100-value)
        self.line.dash_offset = int(value)
    ...
    def on_touch_move(self, touch):
        if self.collide_point(touch.pos[0], touch.pos[1]):
            ...
            if c:
                self.points[(c-1)*2] = touch.pos[0] - self.pos[0]  #this gets relative position in self window
